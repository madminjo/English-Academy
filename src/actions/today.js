...
const getLessonPrompt = (topicName, targetDay, lang) => {
  ...
  const context = isGerman ? "Берлине и местной культуре" : "Лос-Анджелесе, калифорнийском солнце и серфинге";
...
module.exports = (bot) => {
  bot.action('action_today', async (ctx) => {
    ...
    const statusMsg = await ctx.replyWithHTML('⏳ <b>Майкл открывает конспект...</b>').catch(() => {});
  ...
  throw new Error("Пустой ответ от ИИ");
    ...
    const keyboard = Markup.inlineKeyboard([
      ...
      [Markup.button.callback('📝 Домашка', 'action_task'), Markup.button.callback('⬅️ В меню', 'action_main_menu')]
    ]);
