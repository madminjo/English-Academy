...
const rawKeys = process.env.GROQ_KEYS || process.env.GROQ_API_KEYS || process.env.GROQ_API_KEY || "";
...
if (apiKeys.length === 0) {
  console.error("❌ [Groq Pool] Критическая ошибка: Ключи GROQ не найдены в переменной окружения .env!");
} else {
  console.log("⚡ [Groq Pool] Сервер успешно запустил пул ротации. Доступно ключей Groq: " + apiKeys.length);
}
 ...
 */
async function generateContentWithRetry(options, retries = apiKeys.length || 4, baseDelay = 2000) {
  let lastError = null;
  ...
  const systemInstruction = options.config?.systemInstruction || options.systemInstruction || "";
  ...
  for (let attempt = 1; attempt <= actualRetries; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const statusText = errorData.error?.message || "Код ответа: " + response.status;
      ...
      const aiText = messageOutput?.content?.[0]?.type === "output_text"
      ...
      if (isRateLimit && attempt < actualRetries) {
        ...
        console.warn("[Groq Pool] Ключ на индексе " + currentAI.index + " поймал лимит 429. Попытка " + attempt + "/" + actualRetries + ". Спим " + delay + "мс и пробуем следующий ключ...");
  ...
  throw new Error("Все доступные ключи из пула Groq вернули ошибку 429. Последний лог: " + lastError.message);
}
